...
from dexmani_policy.agents.obs_encoder.maniflow_encoder import ManiFlowEncoder
...
class ManiFlowAgent(BaseAgent):
    def __init__(
        ...
        self.obs_encoder = ManiFlowEncoder(
            type=encoder_type,
            pc_dim=pc_dim,
            state_dim=state_dim,
            config=pc_encoder_config,
        )
        ...
        obs_seq_len = self.obs_encoder.obs_seq_len
    ...
    def encode_obs_as_condition(self, obs_dict):
        ...
        feat = self.obs_encoder(this_obs_dict)          # (B*T, obs_seq_len, obs_cond_dim)
        feat = feat.reshape(B, -1, self.obs_cond_dim)   # (B, T*obs_seq_len, obs_cond_dim)
        return feat
