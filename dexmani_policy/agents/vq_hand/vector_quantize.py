...
def gumbel_sample(logits, temperature=1., stochastic=False, straight_through=False,
                  dim=-1, training=True):
...
def kmeans(samples, num_clusters, num_iters=10, use_cosine_sim=False):
    ...
    num_codebooks, dim, dtype, device = (
        samples.shape[0], samples.shape[-1], samples.dtype, samples.device,
    )
...
class EuclideanCodebook(nn.Module):
    ...
    def forward(self, x, sample_codebook_temp=None, freeze_codebook=False):
        ...
        dist = -(
            rearrange(x2, 'h n -> h n 1') +
            rearrange(e2, 'h c -> h 1 c') +
            xy
